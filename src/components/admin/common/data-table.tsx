...
export default function DataTable<T extends Record<string, unknown>>({
  ...
  if (loading) {
    return (
      <Card className="group relative border border-blue-500/30 bg-gradient-to-br from-[#23243A]/50 to-[#181A20]/50 backdrop-blur-sm">
                      ...
                      <div className="flex space-x-2">
                        {actions.map((action, actionIndex) => (
                          <Button
                            ...
                            size="sm"
                            className={`${getActionVariantStyles(action.variant)} transition-all duration-200`}
                          >
                            {action.icon && (
