...
export default function FAQCard({
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: -20 }}
      className="group relative rounded-xl border border-blue-500/30 bg-gradient-to-br from-[#23243A]/50 to-[#181A20]/50 p-4 backdrop-blur-sm transition-all duration-300 hover:scale-105 hover:border-blue-500/50 hover:shadow-lg hover:shadow-blue-500/20"
    >
      {/* Card glow effect */}
      <div className="absolute inset-0 rounded-xl bg-gradient-to-br from-blue-600/5 to-purple-600/5 opacity-0 blur-sm transition-opacity duration-300 group-hover:opacity-100" />
        ...
        {faq.pages.map((pageId) => {
          ...
          return (
            <Badge
              key={pageId}
              variant="outline"
              className="border border-blue-500/30 bg-gradient-to-r from-blue-600/20 to-purple-600/20 text-xs text-blue-300"
            >
              {page?.label || pageId}
            </Badge>
          );
      ...
      <div className="relative flex items-center justify-between text-xs">
        <span
          className={`rounded-full px-3 py-1 font-medium ${
            faq.is_active
              ? 'border border-green-500/30 bg-gradient-to-r from-green-600/20 to-green-800/20 text-green-400'
              : 'border border-red-500/30 bg-gradient-to-r from-red-600/20 to-red-800/20 text-red-400'
          }`}
        >
